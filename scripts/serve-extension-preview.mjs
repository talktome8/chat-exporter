...
createServer(async (request, response) => {
  ...
  try {
    ...
    if (pathname === "/popup.html") {
      ...
      const dark = url.searchParams.get("theme") === "dark";
      const stub = `<script>window.matchMedia=()=>({matches:${dark},addEventListener(){}});window.chrome={storage:{local:{get:async()=>({settingsV2:{dismissedWidgetTip:true}}),set:async()=>{}}},tabs:{query:async()=>[{id:1,url:'https://gemini.google.com/app/qa'}]},scripting:{executeScript:async()=>[{result:{ok:true,adapter:'gemini',platform:'Gemini',supportStatus:'supported',title:'Japan planning conversation',model:'Gemini 2.5 Pro',messages:[{role:'user',text:'Hello'},{role:'assistant',text:'Hi'}],completeness:'complete',warnings:[],scanMode:'full'}}]},runtime:{sendMessage:async()=>({ok:true}),getManifest:()=>({version:'2.0.0'})}};</script>`;
